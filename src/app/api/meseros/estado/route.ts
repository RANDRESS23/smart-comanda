...
export async function PATCH (request: Request) {
  try {
    ...
    if (estado === null) {
      return NextResponse.json(
        { message: '¡No se encontró el estado a cambiar del mesero!' },
        { status: 400 }
      )
    }

    const meseroEstadoUpdated = await db.estados_Meseros.updateMany({
      data: { id_estado: estado.id_estado, updatedAt: currentDate },
      where: { id_mesero: idMesero }
    })

    if (meseroEstadoUpdated.count === 0) {
      return NextResponse.json(
        { message: '¡No se encontró al mesero para cambiar su estado!' },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json(
      {
        message: '¡El cambio de estado se modificó exitosamente al mesero!'
      },
      { status: 200 }
    )
  } catch (error) {
