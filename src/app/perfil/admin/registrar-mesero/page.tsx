...
import { WaiterTable } from './_components/WaiterTable'
import { RegisterWaiter } from './_components/RegisterWaiter'
import { getMeserosEmails } from '@/services/getMeserosEmails'
import { getCajerosEmails } from '@/services/getCajerosEmails'

export async function generateMetadata (): Promise<Metadata> {
  return {
    title: 'SmartComanda | Registrar Mesero'
  }
}

export default async function RegisterWaiterPage () {
  ...
  return (
    <main className='pt-8 pb-10 px-9 relative w-full font-inter-sans bg-grid-small-black dark:bg-grid-small-white flex flex-col items-center overflow-y-auto'>
      ...
      <span className='bg-clip-text text-transparent bg-gradient-to-b dark:from-white dark:to-neutral-400 from-black/80 to-black text-left font-extrabold text-4xl z-10 pb-7 w-full'>
        Registrar <span className='bg-clip-text text-transparent bg-gradient-to-b from-color-pink-primary-dark to-color-pink-primary-accent-dark'>Mesero</span>
      </span>
      <p className='w-full z-10 -mt-3 mb-9 text-p-light dark:text-p-dark text-center lg:text-left'>En esta sección podrás registrar un nuevo mesero, modificar la información de los meseros ya registrados, o simplemente eliminar la cuenta de algún mesero.</p>
      <RegisterWaiter
