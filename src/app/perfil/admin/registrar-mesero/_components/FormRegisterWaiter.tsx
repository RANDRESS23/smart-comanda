...
import { meserosSchema } from '@/app/api/meseros/schema'
...
export const FormRegisterWaiter = ({ onClose, onShoot, supabaseUrl, serviceRolKey }: FormRegisterWaiterProps) => {
  ...
  const {
    ...
    resolver: zodResolver(meserosSchema)
  })
  ...
  const onSubmit: SubmitHandler<FieldValues> = async data => {
    try {
      ...
      const response = await api.post('/meseros/verificar', data)
      ...
      if (response.status === 200) {
        ...
        if (error) {
          ...
          return toast.error('¡Ocurrió un error al registrar el mesero, verifica los datos!.')
        }

        if (user) {
          const response = await api.post('/meseros', { ...data, id_mesero: user.id })

          setMeseros(response.data.meseros as Mesero[])
          toast.success(`¡El mesero con el correo ${data.correo} se registró exitosamente!`)
          onShoot()
          onClose()
        }
      }
  ...
  return (
    <div className='relative font-inter-sans flex flex-col justify-center items-center'>
      <form onSubmit={handleSubmit(onSubmit)} className='w-full flex flex-col gap-5'>
        ...
        <Button
          type="submit"
          text={isLoading ? 'Cargando...' : 'Registrar Mesero'}
          disabled={isLoading}
        />
        <Button
          type="button"
