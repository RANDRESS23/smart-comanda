...
import { cajerosSchema } from '@/app/api/cajeros/schema'
...
export const FormRegisterCashier = ({ onClose, onShoot, supabaseUrl, serviceRolKey }: FormRegisterCashierProps) => {
  ...
  const {
    ...
    resolver: zodResolver(cajerosSchema)
  })
  ...
  const onSubmit: SubmitHandler<FieldValues> = async data => {
    try {
      ...
      const response = await api.post('/cajeros/verificar', data)
      ...
      if (response.status === 200) {
        ...
        if (error) {
          ...
          return toast.error('¡Ocurrió un error al registrar el cajero, verifica los datos!.')
        }

        if (user) {
          const response = await api.post('/cajeros', { ...data, id_cajero: user.id })

          setCajeros(response.data.cajeros as Cajero[])
          toast.success(`¡El cajero con el correo ${data.correo} se registró exitosamente!`)
          onShoot()
          onClose()
        }
      }
  ...
  return (
    <div className='relative font-inter-sans flex flex-col justify-center items-center'>
      <form onSubmit={handleSubmit(onSubmit)} className='w-full flex flex-col gap-5'>
        ...
        <Button
          type="submit"
          text={isLoading ? 'Cargando...' : 'Registrar Cajero'}
          disabled={isLoading}
        />
        <Button
          type="button"
